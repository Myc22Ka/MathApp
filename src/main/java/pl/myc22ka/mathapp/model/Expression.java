...
public abstract class Expression implements FunctionInterface {
    ...
    public List<IExpr> getRealRoots() {
        var expr = evaluator.eval(F.Solve(F.Equal(parseExpression(), F.C0), variable).toString());
    ...
    public List<ConditionRoots> getRealConditionRoots(ISymbol symbol) {
        var expr = evaluator
                .eval(F.Solve(F.Equal(parseExpression(), F.C0), variable, F.Rule(F.GenerateConditions, F.True))
                        .toString());
    ...
    public IExpr getDerivative() {
        return evaluator.eval(F.D(parseExpression(), variable).toString());
    }

    @Override
    public IExpr getRange() {
        return evaluator.eval(F.FunctionRange(parseExpression(), variable, F.y).toString());
    }

    @Override
    public IExpr getDomain() {
        return evaluator.eval(F.FunctionDomain(parseExpression(), variable).toString());
    }

    @Override
    public IExpr getIntegral() {
        return evaluator.eval(F.Integrate(parseExpression(), variable).plus(F.C).toString());
    }

    @Override
    public IExpr getFactoredForm() {
        return evaluator.eval(F.Factor(parseExpression()).toString());
    }

    @Override
    public IExpr getFunctionValue(IExpr x) {
        return evaluator.eval(parseExpression().replaceAll(F.Rule(variable, x)).toString());
    }
